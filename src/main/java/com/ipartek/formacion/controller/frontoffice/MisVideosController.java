...
public class MisVideosController extends HttpServlet {
	...
	private void eliminar(HttpServletRequest request, HttpServletResponse response) {
		...
		// aqui se mete el codigo nuevo de comprobacion de q el video es el propietario.
		// en el frontoffice
		videoEliminar = videoDAO.getById(id);
		usuario = (Usuario) session.getAttribute("usuario");

		if (usuario.getId() == videoEliminar.getUsuario().getId()) {
			...
			request.setAttribute("mensaje", new Alert("danger", "ERROR, el video no pertenece al usuario"));
		}
	}
	...
	private void guardar(HttpServletRequest request, HttpServletResponse response) {
		...
		if (violations.isEmpty()) {
			...
			try {
				...
				if (v.getId() == -1) {
					videoDAO.crear(v, usuario.getId(), idCategoria);
					idVideo = v.getId();
				} else {
					videoDAO.modificar(v, usuario.getId(), idCategoria);
				}
	...
	private void listar(HttpServletRequest request, HttpServletResponse response) {
		...
		request.setAttribute("videosVisibles", videoDAO.getAllById(usuario.getId()));
